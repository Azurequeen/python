...
def load_data(start, end,PATH):
    mask_path = PATH['pics_path_true']
    nodules_path = PATH['pics_path_false']
...
def generate_train(lung_path, nodule_path, start, end, batch_size, shuffle = True):
    train = sorted([x for x in os.listdir(lung_path) if x !='.DS_Store'])
...
def generate_val(lung_path, nodule_path, batch_size):
    val = sorted([x for x in os.listdir(lung_path) if x !='.DS_Store'])
    val = val[-1:]
    while True:
