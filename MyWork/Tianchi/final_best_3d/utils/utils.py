...
def create_samples(df_node,img_file,pic_path):    
    ...
    if mini_df.shape[0]>0: # some files may not have a nodule--skipping those 
        ...
        new_overlap = np.zeros_like(img_array)
        new_mask = np.zeros_like(img_array)
        for i_z in range(img_array.shape[0]): # clip prevents going out of bounds in Z
            img = img_array[i_z]
            seg_img, overlap = helpers.get_segmented_lungs(img.copy())
            overlap = skimage.morphology.binary_opening(np.squeeze(overlap), np.ones([5,5]))
            img = normalize(img)
            new_img[i_z] = img
            new_overlap[i_z] = overlap        
        ...
        for node_idx, cur_row in mini_df.iterrows():       
            ...
            new_mask += nodule_mask
        np.save('{}lung/{}.npy'.format(pic_path, patient_id), new_img)
        np.save('{}nodule/{}.npy'.format(pic_path, patient_id), new_mask)
        np.save('{}overlap/{}.npy'.format(pic_path, patient_id), new_overlap)
    return
