...
def dataset_normalized(imgs):
    ...
    for i in range(imgs.shape[0]):
        ...
        imgs_normalized[i] = normalize(imgs_normalized[i])*255
...
def pred_samples(src,img_file,model):
    ...
    img_array = my_PreProc(img_array)
    ...
    for i in range(img_array.shape[0]):
        ...
        img_array_new[i] = p
    #img_array_new = skimage.morphology.binary_opening(np.squeeze(img_array_new), np.ones([3,3,3]))
    #img_array_new[img_array_new == 1] = int(1)
    #img_array_new[img_array_new != 1] = int(0)
    ...
    for v_x in v_xrange:
        for v_y in v_yrange:
            ...
            if np.linalg.norm(center-np.array([p_x,p_y,z]))<=diam:
                mask[int(np.absolute((p_y-origin[1]))/spacing[1]),int((p_x-origin[0])/spacing[0])] = 1.0
    return(mask)
        ...
        for node_idx, cur_row in mini_df.iterrows():       
            ...
            for i, i_z in enumerate(np.arange(int(v_center[2])-1,
                ...
                if img.shape[0] > 512: 
                    ...
                    cv2.imwrite(pic_path + str(patient_id)+'_'+str(node_idx)+'_'+str(i)+ '_m.png',mask*img*255)
                    cv2.imwrite(PATH['model_train_lung'] + str(patient_id)+'_'+str(node_idx)+'_'+str(i)+ '_i.png',img*255)
                    cv2.imwrite(PATH['model_train_nodule'] + str(patient_id)+'_'+str(node_idx)+'_'+str(i)+ '_m.png',mask*255)
