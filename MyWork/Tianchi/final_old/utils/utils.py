...
def cal_dist(var_csv,anno):
    ...
    for index_csv,row_csv in var_csv.iterrows():
        ...
        if mini_ann.shape[0] == 0:
            ratio_min.append(int(2000))
            dist_min.append(int(2000)) 
            ratio_max.append(int(2000))
            dist_max.append(int(2000))           
        else:        
            dist_ = []
            ratio_ = []
            for index_ann,row_ann in mini_ann.iterrows():        
                ...
                dis = numpy.linalg.norm(vec1 - vec2)
                ...
                ratio_.append(dis/row_ann['diameter_mm'])
                dist_.append(np.min(np.array([a1,a2,a3])))
            ratio_min.append(np.min(ratio_))
            dist_min.append(np.min(dist_))  
            ratio_max.append(np.max(ratio_))
            dist_max.append(np.max(dist_))         
    var_csv['ratio_min'] = ratio_min
    var_csv['dist_min'] = dist_min
    var_csv['ratio_max'] = ratio_max
    var_csv['dist_max'] = dist_max    
...
def cal_recall(var_csv,anno):
    ...
    for index_csv,row_csv in anno.iterrows():
        ...
        if mini_ann.shape[0] == 0:
            ...
            ratio_max.append(np.max(ratio_))
            dist_max.append(np.max(dist_))  
    anno['ratio_min'] = ratio_min
    anno['dist_min'] = dist_min
    anno['ratio_max'] = ratio_max
    anno['dist_max'] = dist_max  
    return anno
